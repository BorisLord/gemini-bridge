...
class CrossPlatformCookieExtractor:
    ...
    def _get_browser_profile_paths(self, browser_name: str) -> Dict[str, Any]:
        ...
        if self.is_windows:
            ...
            if browser_name == "chrome":
                ...
                # New (Network/Cookies) then old (Default/Cookies) Chrome layouts.
                possible_paths = [
    ...
    def _decrypt_chrome_cookie_value(self, encrypted_value: bytes, local_state_path: str) -> Optional[str]:
        ...
        try:
            ...
            # Strip 'DPAPI' prefix (5 bytes) before DPAPI unwrap.
            encrypted_key = encrypted_key[5:]
            key = win32crypt.CryptUnprotectData(encrypted_key, None, None, None, 0)[1]
            logger.info(f"Decrypted key length: {len(key)}")

            # Cookie format: version(3) + nonce(12) + ciphertext + tag(16); min = 32.
            ...
            if version != b'v10' and version != b'v11':
                # Pre-v10 Chrome wrapped the value with raw DPAPI.
    ...
    def _get_chromium_cookies_direct(self, cookies_db_path: str, local_state_path: str = None) -> Optional[list]:
        try:
            ...
            # Copy the DB out — the original is locked while the browser runs.
            import tempfile
            import shutil
    ...
    def get_cookies_with_fallback(self, browser_name: str) -> Optional[Any]:
        ...
        # Windows Chromium needs direct DB + DPAPI; Firefox has no second-chance path.
        if self.is_windows and browser_name in ("chrome", "brave", "edge"):
            browser_paths = self._get_browser_profile_paths(browser_name)
            cookies_db_path = browser_paths.get("cookies_db")
            ...
            if cookies_db_path and os.path.exists(cookies_db_path):
                ...
                if cookies:
                    logger.info(f"Retrieved {browser_name} cookies via direct DB access")
                    return cookies
...
def get_cookie_from_browser(service: Literal["gemini"]) -> Optional[tuple]:
    ...
    if service == "gemini":
        ...
        if secure_1psid and secure_1psidts:
            # Empty values usually mean Windows encrypted them and decrypt failed.
